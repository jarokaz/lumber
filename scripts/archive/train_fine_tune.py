...
def VGG16Trunk(image_shape, input_name, optimizer, loss, metrics, fine_tuning=False):
...
def XceptionTrunk(image_shape, input_name, optimizer, loss, metrics, fine_tuning=False):
...
def main(model, train_file, valid_file, ckpt_folder, optimizer, batch_size, max_steps,  lr, l2, fine_tuning):
    ...
    if model == 'VGG16Trunk':
        model_fn =  VGG16Trunk(IMAGE_SHAPE, INPUT_NAME, optimizer, loss, metrics, fine_tuning) 
    elif model == 'XceptionTrunk':
        model_fn = Xception(IMAGE_SHAPE, INPUT_NAME, optimizer, loss, metrics, fine_tuning)
...
if __name__ == '__main__':
    ...
    if args.model not in ['VGG16Trunk', 'XceptionTrunk']:
